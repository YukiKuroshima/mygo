...
class Jedi1Parsers extends RegexParsers {
  ...
  def declaration: Parser[Declaration] = "def" ~ identifier ~ "=" ~ expression ^^ {
    case "def" ~ identifier ~ "=" ~ expression => Declaration(identifier, expression)
  }
  ...
  def inequality: Parser[Expression] = sum ~ opt(("<" | ">" | "!=") ~ sum) ^^ {
    case sum ~ None  => sum
    case inequ ~ Some("<" ~ sum) => FunCall(Identifier("less"), List(inequ, sum))
    case inequ ~ Some(">" ~ sum) => FunCall(Identifier("more"), List(inequ, sum))
    case inequ ~ Some("!=" ~ sum) => FunCall(Identifier("unequals"), List(inequ, sum))
  }
  ...
  def operands: Parser[List[Expression]] = "(" ~ opt(expression ~ rep("," ~> expression)) ~ ")" ^^ {
    // case "(" ~ None ~ ")" => throw new Exception("Parse operands error")
    case "(" ~ Some(exp ~ Nil) ~ ")" => List(exp)
    case "(" ~ Some(exp ~ more) ~ ")" => exp :: more
    case _ => throw new Exception("Parse operands error bad")
  }
}
